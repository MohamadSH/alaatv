@foreach( $employeeTimeSheets as $employeeTimeSheet)
    <tr>
        ...
        <td>
            <div class = "btn-group">
                ...
                <ul class = "dropdown-menu" role = "menu" id = "{{$employeeTimeSheet->id}}">
                    ...
                    @endpermission @permission((Config::get('constants.REMOVE_EMPLOPYEE_WORK_SHEET')))
                    <li>
        ...
        <td>@if(isset($employeeTimeSheet->timeSheetLock)) {{ $employeeTimeSheet->timeSheetLock }} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
        @permission((Config::get('constants.LIST_EMPLOPYEE_WORK_SHEET')))
        <td>@if(isset($employeeTimeSheet->isPaid)) {{ $employeeTimeSheet->isPaid }} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
        @endpermission
        <td>@if(isset($employeeTimeSheet->workdaytype_id)) {{ $employeeTimeSheet->workdaytype->displayName }} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
        <td>@if(isset($employeeTimeSheet->employeeComment[0])) {!!   $employeeTimeSheet->employeeComment !!} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--info"> ندارد </span> @endif</td>
        <td>@if(isset($employeeTimeSheet->managerComment[0])) {!! $employeeTimeSheet->managerComment !!} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--info"> ندارد </span> @endif</td>
        @permission((Config::get('constants.LIST_EMPLOPYEE_WORK_SHEET')))
        <td>@if(isset($employeeTimeSheet->updated_at)) {{ $employeeTimeSheet->updated_at }} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
        <td>@if(isset($employeeTimeSheet->modifier_id)) {{ $employeeTimeSheet->getModifierFullName() }} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
        <td>@if(isset($employeeTimeSheet->created_at)) {{ $employeeTimeSheet->created_at }} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
        @endpermission
    </tr>
@endforeach
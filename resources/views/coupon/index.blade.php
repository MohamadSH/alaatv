...
@foreach($coupons as $coupon)
    <tr>
        ...
        <th></th>
        <td>@if(isset($coupon->name) && strlen($coupon->name) > 0){{$coupon->name}} @else <span
                    class="label label-sm label-danger"> درج نشده </span> @endif</td>

        <td>@if(isset($coupon->code) && strlen($coupon->code) > 0){{$coupon->code}} @else <span
                    class="label label-sm label-danger"> درج نشده </span> @endif</td>

        <td>@if(isset($coupon->discount) && strlen($coupon->discount) > 0){{$coupon->discount}} @else <span
                    class="label label-sm label-danger"> درج نشده </span> @endif</td>
        <td>@if(isset($coupon->maxCost[0])){{$coupon->maxCost}} @else <span
                    class="label label-sm label-warning"> ندارد </span> @endif</td>

        <td>@if(isset($coupon->usageLimit) && strlen($coupon->usageLimit) > 0){{$coupon->usageLimit}} @else <span
                    class="label label-sm label-info"> نامحدود </span> @endif</td>

        <td>@if(isset($coupon->usageNumber) && strlen($coupon->usageNumber) > 0){{$coupon->usageNumber}} @else <span
                    class="label label-sm label-danger"> درج نشده </span> @endif</td>
        ...
        <td>
            <div class="btn-group">
                ...
                <!-- static -->
                @permission((Config::get('constants.REMOVE_COUPON_ACCESS')))
                <div id="static-{{$coupon->id}}" class="modal fade" tabindex="-1" data-backdrop="static"
                     data-keyboard="false">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" data-dismiss="modal" class="btn green"
                                onclick="removeCoupon('{{action("CouponController@destroy" , $coupon)}}');">بله
                        </button>
                    </div>
                </div>
                @endpermission

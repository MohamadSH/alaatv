...
@foreach($coupons as $coupon)
    <tr>
        ...
        <th></th>
        <td>@if(isset($coupon->name) && strlen($coupon->name) > 0){{$coupon->name}} @else <span class="label label-sm label-danger"> درج نشده </span> @endif</td>

        <td>@if(isset($coupon->code) && strlen($coupon->code) > 0){{$coupon->code}} @else <span class="label label-sm label-danger"> درج نشده </span> @endif</td>

        <td>@if(isset($coupon->discount) && strlen($coupon->discount) > 0){{$coupon->discount}} @else <span class="label label-sm label-danger"> درج نشده </span> @endif</td>
        <td>@if(isset($coupon->maxCost[0])){{$coupon->maxCost}} @else <span class="label label-sm label-warning"> ندارد </span> @endif</td>

        <td>@if(isset($coupon->usageLimit) && strlen($coupon->usageLimit) > 0){{$coupon->usageLimit}} @else <span class="label label-sm label-info"> نامحدود </span> @endif</td>

        <td>@if(isset($coupon->usageNumber) && strlen($coupon->usageNumber) > 0){{$coupon->usageNumber}} @else <span class="label label-sm label-danger"> درج نشده </span> @endif</td>

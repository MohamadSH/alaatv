...
@foreach($mbtiAnswers as $mbtiAnswer)
    <tr>
        ...
        <td>@if(isset($mbtiAnswer->user->mobile))  {{$mbtiAnswer->user->mobile}} @else
        ...
        <td>@if(strlen($mbtiAnswer->getUserOrderInfo("productName"))>0) {{$mbtiAnswer->getUserOrderInfo("productName")}} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger">درج نشده </span> @endif</td>
        <td>@if(strlen($mbtiAnswer->getUserOrderInfo("orderStatus"))>0)
                <span class = "m-badge m-badge--wide label-sm m-badge--success"> {{$mbtiAnswer->getUserOrderInfo("orderStatus")}} </span> @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger">درج نشده </span> @endif</td>
        <td>
            <div class = "btn-group">
                ...
                <ul class = "dropdown-menu" role = "menu">
                    <li>
                        <a target = "_blank" href = "{{action("Web\MbtianswerController@create" , ["action"=>"correctExam","user_id"=>$mbtiAnswer->user->id])}}">مشاهده پاسخنامه</a>
                    </li>
                </ul>
            </div>
        </td>

...
@foreach($userbons as $userbon)
    <tr id = "{{$userbon->id}}">
        <th></th>
        <td id = "userBonFullName_{{$userbon->id}}">@if(strlen($userbon->user->reverse_full_name) > 0)
                <a target = "_blank" href = "{{action("Web\UserController@edit" , $userbon->user)}}">{{$userbon->user->reverse_full_name}}</a> @else
        ...
        <td>@if(isset($userbon->userbonstatus)) @if($userbon->userbonstatus->id == Config::get("constants.USERBON_STATUS_ACTIVE"))
                ...
                <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
        <td>@if(isset($userbon->orderproduct)) {{$userbon->orderproduct->product->name}} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>

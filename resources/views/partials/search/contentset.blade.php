@if($items->isNotEmpty())
    ...
    {{--<a href="@if($contentset->contents->where("contenttype_id",Config::get("constants.CONTENT_TYPE_VIDEO") )->where("enable" , 1)->where('validSince' , '<' , Carbon\Carbon::createFromFormat('Y-m-d H:i:s', Carbon\Carbon::now())->timezone('Asia/Tehran'))->isNotEmpty()){{action("ContentController@show", $contentset->contents->where("contenttype_id",Config::get("constants.CONTENT_TYPE_VIDEO") )->where("enable" , 1)->where('validSince' , '<' , Carbon\Carbon::createFromFormat('Y-m-d H:i:s', Carbon\Carbon::now())->timezone('Asia/Tehran'))->sortBy("pivot.order")->last()->id)}}--}}
    {{--@elseif($contentset->contents->where("contenttype_id",Config::get("constants.CONTENT_TYPE_PAMPHLET") )->where("enable" , 1)->where('validSince' , '<' , Carbon\Carbon::createFromFormat('Y-m-d H:i:s', Carbon\Carbon::now())->timezone('Asia/Tehran'))->isNotEmpty()) {{action("ContentController@show", $contentset->contents->where("contenttype_id",Config::get("constants.CONTENT_TYPE_PAMPHLET") )->where("enable" , 1)->where('validSince' , '<' , Carbon\Carbon::createFromFormat('Y-m-d H:i:s', Carbon\Carbon::now())->timezone('Asia/Tehran'))->sortBy("pivot.order")->last()->id)}}--}}
    {{--@else #@endif">--}}
                ...
                <div class="cbp-item">
                    <div class="cbp-caption">
                        <div class="cbp-caption-defaultWrap">
                            <img src="{{(isset($contentset->photo))?$contentset->photo."?w=460&h=259":"https://cdn.sanatisharif.ir/upload/contentset/departmentlesson/".$contentset->id.".jpg?w=460"}}"
                                 alt=" @if(isset($contentset->name)) {{$contentset->name}} @endif"></div>
                        <div class="cbp-caption-activeWrap">
                            <div class="cbp-l-caption-alignCenter">
                                <div class="cbp-l-caption-body">
                                    <a href="@if($contentset->contents->where("contenttype_id",Config::get("constants.CONTENT_TYPE_VIDEO") )->where("enable" , 1)->where('validSince' , '<' , Carbon\Carbon::createFromFormat('Y-m-d H:i:s', Carbon\Carbon::now())->timezone('Asia/Tehran'))->isNotEmpty()){{action("ContentController@show", $contentset->contents->where("contenttype_id",Config::get("constants.CONTENT_TYPE_VIDEO") )->where("enable" , 1)->where('validSince' , '<' , Carbon\Carbon::createFromFormat('Y-m-d H:i:s', Carbon\Carbon::now())->timezone('Asia/Tehran'))->sortBy("pivot.order")->last()->id)}}
                                    @elseif($contentset->contents->where("contenttype_id",Config::get("constants.CONTENT_TYPE_PAMPHLET") )->where("enable" , 1)->where('validSince' , '<' , Carbon\Carbon::createFromFormat('Y-m-d H:i:s', Carbon\Carbon::now())->timezone('Asia/Tehran'))->isNotEmpty()) {{action("ContentController@show", $contentset->contents->where("contenttype_id",Config::get("constants.CONTENT_TYPE_PAMPHLET") )->where("enable" , 1)->where('validSince' , '<' , Carbon\Carbon::createFromFormat('Y-m-d H:i:s', Carbon\Carbon::now())->timezone('Asia/Tehran'))->sortBy("pivot.order")->last()->id)}}
                                    @else #@endif" class="cbp-l-caption-buttonLeft btn red uppercase ">تماشای این
                                        دوره</a>
                    ...
                    <div class="cbp-l-grid-projects-title text-center" style="font-size: small">
                        ...
                        @elseif($contentset->contents->where("contenttype_id",Config::get("constants.CONTENT_TYPE_PAMPHLET") )->where("enable" , 1)->where('validSince' , '<' , Carbon\Carbon::createFromFormat('Y-m-d H:i:s', Carbon\Carbon::now())->timezone('Asia/Tehran'))->isNotEmpty()) {{action("ContentController@show", $contentset->contents->where("contenttype_id",Config::get("constants.CONTENT_TYPE_PAMPHLET") )->where("enable" , 1)->where('validSince' , '<' , Carbon\Carbon::createFromFormat('Y-m-d H:i:s', Carbon\Carbon::now())->timezone('Asia/Tehran'))->sortBy("pivot.order")->last()->id)}}
                        @else #@endif">
                            @if(isset($contentset->name)) {{$contentset->name}}@else <span
                                    class="font-red">بدون عنوان</span> @endif
                        </a>
                    </div>

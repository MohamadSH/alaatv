...
<div id="js-grid-juicy-projects" class="cbp">
        ...
        <div class="cbp-item hamayesh ">
            <div class="cbp-caption">
                <div class="cbp-caption-defaultWrap">
                    <img src="https://takhtekhak.com/image/4/256/256/arabi-naseh_20180206093949.jpg"
                         alt="جمع بندی عربی کنکور میلاد ناصح زاده">
                </div>
                <div class="cbp-caption-activeWrap">
                    <div class="cbp-l-caption-alignCenter">
                        <div class="cbp-l-caption-body">
                            ...
                            <a href="{{action("ProductController@showLive" , 183)}}"
                               class="btn green uppercase">تماشا</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    @if(isset($withAd) && $withAd)
        <div class="cbp-item hamayesh ">
            ...
            <div class="cbp-l-grid-projects-desc text-center bold font-red product-potfolio-no-cost">قیمت: پس از انتخاب
                محصول
            </div>
    ...
    @foreach($products as $product)
        <div class="cbp-item @if(str_contains($product->name, 'همایش')) hamayesh @elseif(str_contains($product->name, 'اردو')) ordoo @elseif(str_contains($product->name, 'کتاب')) ketab @endif ">
            <div class="cbp-caption">
                <div class="cbp-caption-defaultWrap">
                    @if(strlen($product->image)>0)<img
                            src="{{ route('image', ['category'=>'4','w'=>'256' , 'h'=>'256' ,  'filename' =>  $product->image ]) }}"
                            alt="عکس محصول@if(isset($product->name[0])) {{$product->name}} @endif"> @endif
                </div>
                <div class="cbp-caption-activeWrap">
                    <div class="cbp-l-caption-alignCenter">
                        <div class="cbp-l-caption-body">
                            {{--ToDo : add get parameter to action--}}
                            {{--                            <a href="{{action("ProductController@showPartial" , $product)}}" class="cbp-singlePage cbp-l-caption-buttonLeft btn red uppercase" >اطلاعات بیشتر</a>--}}
                            <a href="{{action("ProductController@show" , $product)}}" class="btn red uppercase">اطلاعات
                                بیشتر</a>
            ...
            @if(strlen($product->name)>0)
                <div class="cbp-l-grid-projects-title uppercase text-center">{{$product->name}}</div>@endif
            ...
            @elseif($product->basePrice == 0)
                <div class="cbp-l-grid-projects-desc text-center bold font-blue product-potfolio-no-cost">قیمت: پس از
                    انتخاب محصول
                </div>

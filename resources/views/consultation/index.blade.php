...
@else
    ...
    @foreach( $consultations as $consultation)
        <tr>
            <th></th>
            <td>@if(isset($consultation->name) && strlen($consultation->name)>0) {{ $consultation->name }} @else
            ...
            <td>@if(isset($consultation->major->id)) @if(strlen($consultation->major->name)>0) {{ $consultation->major->name }} @else {{ $consultation->major->id }} @endif @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
            <td>@if(isset($consultation->videoPageLink) && strlen($consultation->videoPageLink)>0)
                    <a href = "{{ $consultation->videoPageLink }}">دانلود</a> @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
            <td>@if(isset($consultation->textScriptLink) && strlen($consultation->textScriptLink)>0)
                    <a href = "{{ $consultation->textScriptLink }}">دانلود</a> @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
            <td>
            ...
            <td>
                <div class = "btn-group">
                    ...
                    <ul class = "dropdown-menu" role = "menu">
                        ...
                        @endpermission @permission((Config::get('constants.REMOVE_CONSULTATION_ACCESS')))
                        <li>
                    ...
                    <!-- static -->@permission((Config::get('constants.REMOVE_CONSULTATION_ACCESS')))

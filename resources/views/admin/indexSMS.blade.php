...
@section("content")
    <div class="row">
        ...
        {{--<!-- /.modal -->--}}
        <div class="col-md-12">
            <p class="list-group-item bold bg-font-dark"  style="text-align: justify; background: #489fff"><i class="fa fa-database"></i>
                @if($smsCredit === false) خطا در دریافت اطالاعات @else   اعتبار فعلی شما <span id="smsCredit">{{number_format($smsCredit)}}</span> ریال برابر با <span id="smsCreditNumber_2">{{number_format(ceil($smsCredit/Config::get('constants.COST_PER_SMS_2')))}}</span> پیامک اپراتور (1000) برابر با <span id="smsCreditNumber_1">{{number_format(ceil($smsCredit/Config::get('constants.COST_PER_SMS_1')))}}</span> پیام کاپراتور(5000) برابر با <span id="smsCreditNumber_3">{{number_format(ceil($smsCredit/Config::get('constants.COST_PER_SMS_3')))}}</span>  پیامک اپراتور (sim) @endif
            </p>
        </div>
        <div class="col-md-12">
                            ...
                            {!! Form::open(['method' => 'POST', 'action' => 'HomeController@sendSMS' , 'class'=>'' , 'id'=>'sendSmsForm']) !!}
                                {!! Form::textarea('message', null, ['class' => 'form-control' , 'id' => 'smsMessage', 'placeholder' => 'متن پیامک' , 'rows' => 3]) !!}
                                    ...
                                    <span class="help-block">
                                        طول پیام: (<span style="color: red;"><span id="smsNumber">1</span> پیامک</span> ) <span id="smsWords">70</span>    کارکتر باقی مانده تا پیام بعدی
                                    </span>
                            {!! Form::close() !!}
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <a type="button" class="btn green-soft" data-target="#smsConformation" data-toggle="modal" id="sendSmsButton">ارسال</a>
                                    ...
                                    <div class="form-body" style="background: #e7ecf1">
                                            ...
                                            <div class="form-group">
                                                <div class="ccl-mg-12 col-md-12">
                                                    <button type="button" id="filterButton" class="btn btn-lg bg-font-dark" style="background: #489fff">فیلتر</button>
                                                    <img class="hidden" id="sms-portlet-loading" src="{{Config::get('constants.FILTER_LOADING_GIF')}}" alt="loading"  width="5%">
                                                </div>
                                            </div>
                                    </div>
                                {!! Form::close() !!}

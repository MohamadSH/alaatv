...
{{--        @if(isset($columns) && in_array("name",$columns))--}}
            <td>@if(isset($item->name[0])) <a target="_blank" href="{{action("ContentController@show" , $item)}}"> {{ $item->name }} </a> @else <span class="label label-sm label-danger"> درج نشده </span> @endif </td>
        {{--@endif--}}
...
{{--        @if(isset($columns) && in_array("enable",$columns))--}}
        ...
        {{--@if(isset($columns) && in_array("contentType",$columns))--}}
            <td>@if(isset($item->contenttype->id)) {{$item->contenttype->displayName}} @else<span class="label label-sm label-danger">  نامشخص </span>@endif</td>
        {{--@endif--}}
        {{--@if(isset($columns) && in_array("file",$columns))--}}
            <td>
                @if($item->files->count() == 1)
                    <a target="_blank" href="" class="btn btn-circle green btn-outline btn-sm"><i class="fa fa-download"></i> دانلود </a>
                @else
                    <div class="btn-group">
        ...
        {{--@if(isset($columns) && in_array("description",$columns))--}}
            <td style="text-align: center">
                @if(isset($item->description[0]))
                    <button class="btn blue" data-target="#static-description-{{$item->id}}" data-toggle="modal">نمایش </button>
                    <div id="static-description-{{$item->id}}" class="modal fade" tabindex="-1" data-backdrop="static" data-keyboard="false">
        ...
        {{--@if(isset($columns) && in_array("validSince",$columns))--}}
            <td class="center">@if(isset($item->validSince)) {{ $item->validSince_Jalali() }} @else <span class="label label-sm label-danger"> درج نشده </span> @endif </td>
        {{--@endif--}}
        {{--@if(isset($columns) && in_array("created_at",$columns))--}}
            <td class="center">@if(isset($item->created_at)) {{ $item->CreatedAt_Jalali() }}  @else <span class="label label-sm label-danger"> درج نشده </span> @endif </td>
        {{--@endif--}}
        {{--@if(isset($columns) && in_array("updated_at",$columns))--}}
            <td class="center">@if(isset($item->updated_at)) {{ $item->UpdatedAt_Jalali() }}  @else <span class="label label-sm label-danger"> درج نشده </span> @endif </td>
        {{--@endif--}}
            ...
            <div class="btn-group">
                ...
                <div id="static-{{$item->id}}" class="modal fade" tabindex="-1" data-backdrop="static" data-keyboard="false">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" data-dismiss="modal"  class="btn green" onclick="removeContent('{{action("ContentController@destroy" , $item)}}');" >بله</button>
                    </div>
                </div>
                {{--@endpermission--}}
            </div>

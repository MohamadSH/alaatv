...
@else
    ...
    @foreach($items as $item)
        <tr>
            ...
            <td>@if(isset($item->name[0]))
                    <a target = "_blank" href = "{{action("Web\ContentController@show" , $item)}}"> {{ $item->name }} </a> @else
            ...
            <td>@if(isset($item->contenttype->id)) {{$item->contenttype->displayName}} @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger">  نامشخص </span>@endif</td>
            {{--@endif--}}
            {{--@if(isset($columns) && in_array("file",$columns))--}}
            <td>
                @if($item->files->count() == 1)
                    <a target = "_blank" href = "" class = "btn btn-circle green btn-outline btn-sm">
            ...
            <td class = "center">@if(isset($item->validSince)) {{ $item->validSince_Jalali() }} @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif </td>
            {{--@endif--}}
            {{--@if(isset($columns) && in_array("created_at",$columns))--}}
            <td class = "center">@if(isset($item->created_at)) {{ $item->CreatedAt_Jalali() }}  @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif </td>
            {{--@endif--}}
            {{--@if(isset($columns) && in_array("updated_at",$columns))--}}
            <td class = "center">@if(isset($item->updated_at)) {{ $item->UpdatedAt_Jalali() }}  @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif </td>
            {{--@endif--}}
            ...
            <td>
                <div class = "btn-group">
                    ...
                    <ul class = "dropdown-menu" role = "menu">
                        ...
                        @endpermission @permission((Config::get('constants.REMOVE_EDUCATIONAL_CONTENT_ACCESS')))
                        <li>

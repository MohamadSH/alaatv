...
@foreach( $attributes as $attribute)
    <tr>
        <th></th>
        <td>@if(isset($attribute->name) && strlen($attribute->name)>0 ) {{ $attribute->name }}  @else
        ...
        <td>@if(isset($attribute->displayName) && strlen($attribute->displayName)>0 ) {{ $attribute->displayName }}  @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
        <td>@if(isset($attribute->attributecontrol_id)) {{ $attribute->attributecontrol->name }}  @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
        ...
        <td>
            <div class = "btn-group">
                ...
                <ul class = "dropdown-menu" role = "menu">
                    ...
                    @endpermission @permission((config('constants.REMOVE_ATTRIBUTE_ACCESS')))
                    <li>
                ...
                <!-- static -->@permission((config('constants.REMOVE_ATTRIBUTE_ACCESS')))

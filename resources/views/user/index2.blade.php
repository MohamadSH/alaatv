@foreach($items as $item)
    <tr id = "{{$item->id}}">
        ...
        <td>@if(isset($item->major->id) && strlen($item->major->name)>0 ) {{$item->major->name}} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--warning"> درج نشده </span> @endif</td>
        <td>
            <span class = "bold">شماره شخصی:</span>
            @if(isset($item->mobile) && strlen($item->mobile)>0) {{ $item->mobile}} @else
                ...
                @foreach($item->contacts as $contact)
                    @if(isset($contact->relative->id))
                        @if(strcmp($contact->relative->name,'father') == 0)
                            ...
                            <span class = "bold">شماره پدر</span>
                            <br>
                            @if(!$contact->phones->isEmpty())
                                ...
                                @endforeach
                            @else
                                <span class = "m-badge m-badge--wide label-sm m-badge--info">بدون شماره</span>
                        ...
                        @elseif(strcmp($contact->relative->name,'mother') == 0)
                            ...
                            <span class = "bold">شماره مادر</span>
                            <br>
                            @if(!$contact->phones->isEmpty())
                                @foreach($contact->phones as $phone)
                                    {{$phone->phoneNumber}}
        ...
        <td>@if(isset($item->nationalCode) && strlen($item->nationalCode)>0) {{ $item->nationalCode }} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
        <td>@if(isset($item->city) && strlen($item->city)>0) {{ $item->city }} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--warning"> درج نشده </span> @endif</td>
    </tr>
@endforeach
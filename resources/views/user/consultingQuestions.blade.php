...
@section("title")
    <title>تخته خاک|سؤالات مشاوره ای من</title>
@endsection
                        ...
                        @else
                            @foreach($questions as $question)
                                <tr>
                                    ...
                                    <td> @if(strlen($question->file) > 0)  <a target="_blank" href="{{action("HomeController@download" , ["content"=>"سؤال مشاوره ای","fileName"=>$question->file ])}}" id="link_{{$counter}}" class="btn btn-icon-only blue"><i class="fa fa-download"></i></a>@else <span class="label label-sm label-danger"> بدون فایل </span> @endif </td>
                                    ...
                                    <td>@if(isset($question->useruploadstatus->id)) @if(strcmp($question->useruploadstatus->name , "done")==0) <span class="label label-sm label-success">پاسخ داده شده </span> @elseif(strcmp($question->useruploadstatus->name , "processing")==0) <span class="label label-sm label-warning">در حال بررسی مشاور </span> @elseif(strcmp($question->useruploadstatus->name , "pending")==0) <span class="label label-sm label label-default">مشاور ندیده</span> @endif

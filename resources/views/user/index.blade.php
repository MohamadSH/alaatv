...
@foreach( $users as $user)
    <tr>
        <th></th>
        <td id="userFullName_{{$user->id}}">@if(isset($user->lastName[0])) {{$user->lastName}} @else <span
                    class="label label-sm label-danger">نامشخص </span> @endif</td>
        <td>@if(isset($user->firstName[0])) {{$user->firstName}} @else <span
                    class="label label-sm label-danger">نامشخص </span> @endif</td>
        ...
        <td>
            <div class="btn-group">
                ...
                <ul class="dropdown-menu" role="menu" id="{{$user->id}}">
                    ...
                    <li>
                        <a target="_blank" href="{{action("OrderController@create" , ["customer_id"=>$user->id])}}"><i

...
@section("title")
    <title>تخته خاک|سفارش های من</title>
@endsection
...
@section("content")
    <div class="row">
        <div class="col-md-12">
            ...
            @if(isset($debitCard))
            <p class="list-group-item  bg-blue-soft bg-font-blue-soft" style="text-align: justify;"> شماره کارت برای واریز کارت به کارت مبلغ: <span dir="ltr">{{$debitCard->cardNumber}}</span>
                به نام @if(!isset($debitCard->user->firstName) && !isset($debitCard->user->lastName)) کاربر ناشناس @else @if(isset($debitCard->user->firstName)) {{$debitCard->user->firstName}} @endif @if(isset($debitCard->user->lastName)) {{$debitCard->user->lastName}} @endif @endif بانک {{$debitCard->bank->name}}
            </p>
            @endif
                ...
                <div class="tabbable-custom ">
                    <div class="portlet-title">
                        ...
                        <ul class="nav nav-tabs ">
                            <li class="active">
                                <a href="#tab_1" data-toggle="tab" class="caption-subject  bold uppercase">لیست سفارش های شما</a>
                            </li>
                            <li>
                                <a href="#tab_2" data-toggle="tab" class="caption-subject  bold uppercase"> لیست تراکنش های شما</a>
                            </li>
                            @if(isset($instalments) && $instalments->isNotEmpty())
                            <li>
                                <a href="#tab_3" data-toggle="tab" class="caption-subject  bold uppercase"> لیست قسط های شما</a>
                            </li>
                            @endif
                        </ul>
                    </div>
                    <div class="portlet-body" >
                        <div class="tab-content">
                            <div class="tab-pane active" id="tab_1">
                                ...
                                @else
                                    <table class="table table-striped table-bordered table-hover dt-responsive" width="100%" id="orders_table">
                                    <div id="onlinePaymentModal" class="modal fade" tabindex="-1" data-backdrop="static" data-keyboard="false">
                                        ...
                                        <div class="modal-body">
                                            <div class="row static-info margin-top-20" id="gatewayDiv">
                                                <div class="form-group">
                                                    <label class="col-md-3 control-label" for="gateway">انتخاب درگاه :</label>
                                                    <div class="col-md-7">
                                            ...
                                            <div class="row static-info margin-top-40" style="text-align: center;">
                                                <span class="label label-success" style="font-size: 15px" id="orderCost"></span>
                                            </div>
                                            ...
                                            <div class="mt-radio-list">
                                                <div class="row static-info margin-top-20">
                                                    <div class="form-group {{ $errors->has('referenceNumber') ? ' has-error' : '' }}">
                                                        <div class="col-md-4">
                                                            <label class="mt-radio mt-radio-outline control-label"> شماره مرجع/ارجاع:
                                                ...
                                                <div class="row static-info margin-top-20">
                                                    <div class="form-group {{ $errors->has('traceNumber') ? ' has-error' : '' }}">
                                                        <div class="col-md-4">
                                                            <label class="mt-radio mt-radio-outline control-label"> شماره پیگیری:
                                    ...
                                    @foreach($orders as $order)
                                        <tr>
                                            ...
                                            <td style="text-align: center;">
                                                @if(isset($order->orderstatus->id) && $order->orderstatus->id == Config::get("constants.ORDER_STATUS_CLOSED"))<span class="label label-success"> {{$order->orderstatus->displayName}}</span>
                                                @elseif(isset($order->orderstatus->id) && $order->orderstatus->id == Config::get("constants.ORDER_STATUS_CANCELED"))<span class="label label-danger"> {{$order->orderstatus->displayName}}</span>
                                            ...
                                            <td style="text-align: center;">
                                                @if(isset($order->paymentstatus->id) && $order->paymentstatus->id == Config::get("constants.PAYMENT_STATUS_PAID")) <span class="label label-success"> {{$order->paymentstatus->displayName}}</span>
                                            ...
                                            <td >@if(isset($order->cost) || isset($order->costwithoutcoupon)){{number_format($order->totalCost() )}} @else <span class="label label-info">بدون مبلغ</span> @endif</td>
                                            <td >@if(isset($order->cost) || isset($order->costwithoutcoupon)){{number_format($order->totalPaidCost() + $order->totalRefund())}} @else <span class="label label-info">بدون مبلغ</span> @endif</td>
                                            <td>@if(isset($order->completed_at)) {{$order->CompletedAt_Jalali()}} @else <span class="label label-info">درج نشده</span> @endif</td>
                                                    ...
                                                    @foreach($order->successfulTransactions as $successfulTransaction)
                                                        ...
                                                        @if($successfulTransaction->getGrandParent() === false)
                                                            @if($successfulTransaction->getCode() === false) - بدون کد @else - {{$successfulTransaction->getCode()}} @endif
                                                                ...
                                                                @endif
                                                        @else
                                                            @if($successfulTransaction->getGrandParent()->getCode() === false) - کد نامشخص @else - {{$successfulTransaction->getGrandParent()->getCode()}} @endif
                                                            - مبلغ: @if($successfulTransaction->getGrandParent()->cost >= 0)
                                                                    {{ number_format($successfulTransaction->getGrandParent()->cost) }} <br>
                                                                    @else
                                                                    {{ number_format(-$successfulTransaction->getGrandParent()->cost) }}(پرداخت) <br>
                                                                    @endif
                                                        @endif
                                            ...
                                            <td>@if($order->pendingTransactions->isEmpty())
                                                ...
                                                @else
                                                    ...
                                                    @foreach($order->pendingTransactions as $pendingTransaction)
                                                        ...
                                                        @if(isset($pendingTransaction->transactionID))  ,شماره تراکنش: {{ $pendingTransaction->transactionID }} مبلغ: {{ number_format($pendingTransaction->cost) }}@endif
                                                        @if(isset($pendingTransaction->traceNumber))  ,شماره پیگیری:{{$pendingTransaction->traceNumber}} مبلغ: {{ number_format($pendingTransaction->cost) }}@endif
                                                        @if(isset($pendingTransaction->referenceNumber))  ,شماره مرجع:{{$pendingTransaction->referenceNumber}} مبلغ: {{ number_format($pendingTransaction->cost) }}@endif
                                                        @if(isset($pendingTransaction->paycheckNumber))  ,شماره چک:{{$pendingTransaction->paycheckNumber}} مبلغ: {{ number_format($pendingTransaction->cost) }}@endif
                                            ...
                                            <td>@if($order->unpaidTransactions->isEmpty())
                                                ...
                                                @else
                                                    ...
                                                    @foreach($order->unpaidTransactions as $instalment)
                                                        @if($instalment->cost) مبلغ: {{ number_format($instalment->cost) }}@endif
                                                        مهلت پرداخت: @if(isset($instalment->deadline_at)){{$instalment->DeadlineAt_Jalali()}}@endif
                                            ...
                                            <td id="cost_{{$order->id}}">@if(isset($order->cost)|| isset($order->costwithoutcoupon)){{number_format($order->debt())}} @else 0 @endif</td>
                                            <td>
                                            ...
                                            <td>@if(isset($order->created_at)) {{$order->CreatedAt_Jalali()}} @else <span class="label label-info">درج نشده</span> @endif</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
                                    ...
                                    <table class="table table-hover table-light">
                                        ...
                                        <tbody>
                                            @forelse($instalments as $transaction)
                                                <tr>
                                                    <td style="direction: ltr; text-align: right">@if(isset($transaction->cost) && strlen($transaction->cost)>0)@if($transaction->cost >= 0) <span id="instalmentCost_{{$transaction->id}}">{{number_format($transaction->cost)}}</span> @else {{number_format(-$transaction->cost)}} @endif @else <span class="label label-sm label-danger"> درج نشده </span>  @endif</td>
                                                    <td>
                                                        <ul class="">
                                                            <li>

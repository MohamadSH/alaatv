                ...
                <div class="dashboard-stat2 ">
                    ...
                    <div class="progress-info" style="font-style:italic">
                        <div class="status" style="font-size: small">
                            5 ساعت جمع بندی و نکته و تست داریم و 1 ساعت تست های پلاس مخصوص پزشکا و مهندسا                        </div>
                ...
                <div class="clearfix">
                    @if(isset($withFilter) && $withFilter)
                        <div id="js-filters-lightbox-gallery2" class="cbp-l-filters-button cbp-l-filters-right">
                            <div data-filter="*" class="cbp-filter-item-active cbp-filter-item btn blue btn-outline uppercase">همه</div>
                            ...
                            <div data-filter=".انسانی" class="cbp-filter-item btn blue btn-outline uppercase">انسانی</div>
                        </div>
                    @endif
                </div>
                <div id="js-grid-lightbox-gallery" class="cbp">
                    @foreach( $landingProducts as $product)
                        <div class="cbp-item @foreach ($product["majors"] as $major) {{ $major }} @endforeach">
                            <a href="{{action("ProductController@show" , $product["product"]->id)}}" class="cbp-caption" data-title="{{$product["product"]->name}}" rel="nofollow">
                                <div class="cbp-caption-defaultWrap">
                                    @if(isset($product["product"]->image[0]))<img src="{{ route('image', ['category'=>'4','w'=>'256' , 'h'=>'256' ,  'filename' =>  $product["product"]->image ]) }}" alt="عکس محصول@if(isset($product["product"]->name[0])) {{$product["product"]->name}} @endif"> </div>@endif
                                <div class="cbp-caption-activeWrap">
                                    <div class="cbp-l-caption-alignLeft">
                                        <div class="cbp-l-caption-body">
                                            <div class="cbp-l-caption-title">ثبت نام در همایش</div>
                                            @if($product["product"]->isFree)
                                                <div class="cbp-l-caption-desc  bold font-red product-potfolio-free" >رایگان </div>
                                            @elseif($product["product"]->basePrice == 0)
                                                <div class="cbp-l-caption-desc  bold font-blue product-potfolio-no-cost">قیمت: پس از انتخاب محصول</div>

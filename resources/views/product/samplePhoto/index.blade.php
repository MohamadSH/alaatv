<div class="portlet light ">
    <div class="portlet-title">
        ...
        <div class="actions">
            <div class="btn-group">
                <a class="btn btn-sm dark dropdown-toggle" data-toggle="modal" href="#createProductPhoto"><i class="fa fa-plus-circle"></i>افزودن نمونه عکس
                ...
                @if(!$productPhotos->isEmpty())
                    @foreach($productPhotos as $photo)
                        <tr class="text-center">
                            <td>@if(isset($photo->order)){{$photo->order}}@else <span class="label label-sm label-danger"> ندارد </span> @endif</td>
                            <td>@if(isset($photo->title[0])){{$photo->title}}@else <span class="label label-sm label-danger"> نا مشخص </span> @endif</td>
                            <td>@if(isset($photo->description[0])){{$photo->description}}@else <span class="label label-sm label-danger"> نا مشخص </span> @endif</td>
                            <td>
                                    ...
                                    <div class="mt-overlay-1">
                                        ...
                                        <div class="mt-overlay">
                                            <ul class="mt-info">
                                                ...
                                                <li>
                                                    <a target="_blank" class="btn default btn-outline" href="{{action("HomeController@download" , ["content"=>"عکس محصول","fileName"=>$photo->file ])}}">
                                                        <i class="icon-link"></i>
                                                    </a>
                                                </li>
                                            </ul>
                            ...
                            <td>@if($photo->enable) <span class="label label-sm label-success"> فعال </span> @else <span class="label label-sm label-danger"> غیر فعال </span>  @endif</td>
                            <td>
                                ...
                                <a class="btn btn-danger"  onclick="removePhoto('{{action("ProductphotoController@destroy" , $photo)}}')" data-id="{{$photo->id}}">
                    ...
                    @endforeach
                @else
                    <tr><td colspan="7" class="bold text-center">عکسی درج نشده است</td></tr>
                @endif
                </tbody>
            </table>
        </div>

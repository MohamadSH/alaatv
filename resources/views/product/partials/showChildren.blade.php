<li style="border:none">
    <label class="bold">
            @if(isset($product->pivot->control_id) && $product->pivot->control_id ==  Config::get("constants.CONTROL_SWITCH"))
                <input name="products[]" value="{{$product->id}}" type="checkbox"  class="hasParent_{{$product->pivot->parent_id}} {{(count($product->children)>0)?"hasChildren":""}} product make-switch  {{(isset($product->pivot->isDefault) && $product->pivot->isDefault)?"isDefault":""}}"  data-size="mini" data-on-color="success" data-off-color="danger" data-on-text="بله" data-off-text="خیر" > {{$product->name}}
            ...
            @else
                <input name="products[]" type="checkbox" value="{{$product->id}}" class="hasParent_{{$product->pivot->parent_id}} {{(count($product->children)>0)?"hasChildren":""}} product icheck {{(isset($product->pivot->isDefault) && $product->pivot->isDefault)?"isDefault":""}}"  data-checkbox="icheckbox_minimal-blue"  >{{$product->name}}
                @if(isset($product["simpleInfoAttributes"]))
                    @foreach($product["simpleInfoAttributes"] as $key => $simpleInfoAttribute)
                        @foreach($simpleInfoAttribute as $key => $info)
                            <span > @if(count($simpleInfoAttribute)>1 && $key < (sizeof($simpleInfoAttribute)-1)) , @endif {{$info["name"]}}</span>

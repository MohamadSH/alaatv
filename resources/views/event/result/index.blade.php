        ...
        <tr>
            ...
            <td>@if(isset($eventresult->user_id))
                    @if(strlen($eventresult->user->lastName) > 0)
                        <a target="_blank"
                           href="{{action("UserController@edit" , $eventresult->user)}}">{{$eventresult->user->lastName}}</a>
                    @else
...
@else
    @foreach($eventresults as $eventresult)
        <tr>
            ...
            <td>@if(isset($eventresult->reportFile[0])) <a target="_blank"
                                                           href="{{action("HomeController@download" , ["content"=>"فایل کارنامه","fileName"=>$eventresult->reportFile ])}}"

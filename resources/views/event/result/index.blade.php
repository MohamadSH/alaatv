    ...
    @foreach($eventresults as $eventresult)
        <tr>
            ...
            <td>@if(isset($eventresult->participationCodeHash) && strlen($eventresult->participationCodeHash)>0) {{$eventresult->participationCodeHash}} @else
                    <span class="label label-sm label-danger">بدون معدل </span> @endif</td>
            <td>
    ...
    @foreach($eventresults as $eventresult)
        <tr>
            ...
            <td>@if(isset($eventresult->user->id))
                    @if(strlen($eventresult->user->reverse_full_name) > 0) <a target="_blank" href = "{{action("Web\UserController@edit" , $eventresult->user)}}">{{$eventresult->user->full_name}}</a> @else
                        <span class="label label-sm label-danger"> کاربر ناشناس </span>@endif
                @endif
            </td>
            <td>@if(isset($eventresult->user->mobile))  {{$eventresult->user->mobile}} @else <span
            ...
            <td>@if(isset($eventresult->reportFile[0])) <a target="_blank" href = "{{action("Web\HomeController@download" , ["content"=>"فایل کارنامه","fileName"=>$eventresult->reportFile ])}}"
                                                           class="btn btn-icon-only green"><i
                            class="fa fa-download"></i></a> @else <span
                        class="label label-sm label-warning"> درج نشده </span> @endif </td>
            <td>@if(isset($eventresult->rank)) {{$eventresult->rank}} @else <span class="label label-sm label-danger">درج نشده </span> @endif
            </td>
            <td style="width: 20%">
            ...
            <td>@if(isset($eventresult->comment) && strlen($eventresult->comment)>0) {{$eventresult->comment}} @else
                    <span class="label label-sm label-info">بدون نظر </span> @endif</td>
            <td>
                @if(isset($eventresult->created_at))
                    {{$eventresult->CreatedAt_Jalali()}}
                @else

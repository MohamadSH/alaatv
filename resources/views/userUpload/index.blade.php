@permission((Config::get('constants.LIST_QUESTION_ACCESS')))
                ...
                <div class="item">
                    <div class="item-head">
                        <div class="item-details">
                            ...
                            <a href="{{action("UseruploadController@show" , $question)}}" class="item-name primary-link">@if(isset($question->user->id)) @if(isset($question->user->firstName) && strlen($question->user->firstName)>0 || isset($question->user->lastName) && strlen($question->user->lastName)>0) @if(isset($question->user->firstName) && strlen($question->user->firstName)>0) {{ $question->user->firstName}} @endif @if(isset($question->user->lastName) && strlen($question->user->lastName)>0) {{$question->user->lastName}} @endif @else <span class="label label-sm label-danger"> کاربر ناشناس </span> @endif @endif</a>
    ...
    @else
                @foreach($questions as $question)
                    <tr id="{{$question->id}}">
                        <th></th>
                        <td>@if(isset($question->user->id)) @if(strlen($question->user->getFullName("lastNameFirst")) > 0) <a target="_blank" href="{{action("UserController@edit" , $question->user)}}">{{$question->user->getFullName("lastNameFirst")}}</a> @else <span class="label label-sm label-danger"> درج نشده </span> @endif @endif</td>
                        <td>@if(strlen($question->title) > 0)  {{$question->title}} @else <span class="label label-sm label-danger">بدون عنوان </span> @endif</td>
                        ...
                        <td style="text-align: center">@if(strlen($question->file) > 0)  <a target="_blank" href="{{action("HomeController@download" , ["content"=>"سؤال مشاوره ای","fileName"=>$question->file ])}}" id="link_{{$counter}}" class="btn btn-icon-only blue"><i class="fa fa-download"></i></a>@else <span class="label label-sm label-danger"> بدون فایل </span> @endif </td>
                        <td  style="width: 20%">

...
@else
    @foreach($questions as $question)
        <tr id = "{{$question->id}}">
            <th></th>
            <td>@if(isset($question->user->id)) @if(strlen($question->user->reverse_full_name) > 0)
                    <a target = "_blank" href = "{{action("Web\UserController@edit" , $question->user)}}">{{$question->user->reverse_full_name}}</a> @else
            ...
            <td style = "text-align: center">@if(strlen($question->file) > 0)
                    ...
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> بدون فایل </span> @endif </td>
            <td style = "width: 20%">

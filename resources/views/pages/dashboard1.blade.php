...
@section("content")
    ...
    <div class="row">
        <div class="col-md-12">
            <h3>{{$sections->where("name" , "konkoor")->first()["displayName"]}}</h3>
            <hr>
            <section class="lessonSlider1 slider" style="width: 95%;margin-top: 0px ; margin-bottom: 15px;">
                @foreach($sections->where("name" , "konkoor")->first()["lessons"] as $lesson)
                    <a href="{{action("HomeController@search" , ["tags" => $lesson["tags"]])}}">
                        <img src="https://www.freeiconspng.com/uploads/orange-pdf-icon-32.png" alt="عکس درس">
                    </a>
                @endforeach
            </section>
        </div>

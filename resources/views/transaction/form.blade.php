    ...
    <div class="row static-info margin-top-20">
        <div class="form-group {{ $errors->has('deadline_at') ? ' has-error' : '' }}">
            <label class="col-md-3 control-label" for="transactionDeadlineAt">
                @if(isset($withCheckbox) && in_array("deadline_at" , $withCheckbox))<input type="checkbox"
                                                                                           name="deadlineAtEnable"
                                                                                           value="1"
                                                                                           id="transactionDeadlineAtEnable">@endif
                مهلت پرداخت:</label>
            <div class="col-md-6">
                <input id="transactionDeadlineAt" type="text" class="form-control"
                       value="@if(isset($deadlineAt)) {{$deadlineAt}} @else {{old('deadline_at')}} @endif"
                       dir="ltr" {{(isset($withCheckbox) && in_array("deadline_at" , $withCheckbox))?"disabled":""}}>
    ...
    <div class="row static-info margin-top-20">
        <div class="form-group {{ $errors->has('completed_at') ? ' has-error' : '' }}">
            <label class="col-md-3 control-label" for="transactionCompletedAt">
                @if(isset($withCheckbox) && in_array("completed_at" , $withCheckbox))<input type="checkbox"
                                                                                            name="completedAtEnable"
                                                                                            value="1"
                                                                                            id="transactionCompletedAtEnable">@endif
                تاریخ پرداخت:</label>
            <div class="col-md-6">
                <input id="transactionCompletedAt" type="text" class="form-control"
                       value="@if(isset($completedAt)) {{$completedAt}} @else {{old('completed_at')}} @endif"
                       dir="ltr" {{(isset($withCheckbox) && in_array("completed_at" , $withCheckbox))?"disabled":""}}>
...
<div class="row static-info margin-top-20">
    <div class="form-group {{ $errors->has('paymentmethod_id') ? ' has-error' : '' }} {{ $errors->has('paymentMethodName') ? ' has-error' : '' }}">
        <label class="col-md-3 control-label"
               for="{{(isset($name["paymentmethod"]))?$name["paymentmethod"]:'paymentmethod_id'}}">روش پرداخت:</label>
        <div class="col-md-6">

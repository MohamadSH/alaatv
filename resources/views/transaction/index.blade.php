...
@foreach($transactions as $transaction)
    <tr id="{{$transaction->id}}">
        ...
        <td>@if(isset($transaction->order->user)) @if(!isset($transaction->order->user->mobile) ) <span
                    class="m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @else {{$transaction->order->user->mobile}} @endif @endif
        </td>
        ...
        <td>
            ...
            @else <span class="m-badge m-badge--wide label-sm m-badge--warning"> ندارد </span>
            @endif
        </td>
        ...
        <td>
            ...
            @if(isset($transaction->order->id))
                <a target = "_blank" class = "btn blue" href = "{{action("Web\OrderController@edit" , $transaction->order)}}">
                    <i class="fa fa-pencil"></i> رفتن به سفارش
                </a>
            @endif
            @endpermission

...
@foreach($transactions as $transaction)
    <tr id = "{{$transaction->id}}">
        ...
        <td>@if($transaction->hasParents())
                <a target = "_blank" href = "{{action('TransactionController@edit' , $transaction->getGrandParent())}}">رفتن به تراکنش</a> @else ندارد @endif
        </td>
        <td>@if(isset($transaction->order->user)) @if(!isset($transaction->order->user->mobile) )
        ...
        <td>@if(isset($transactionOrderproductCost[$transaction->id]))
        ...
        <td>@if(isset($transaction->paymentmethod)) {{$transaction->paymentmethod->displayName}} @else
                <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif
        </td>
        ...
        <td>
            ...
            @if($transaction->cost < 0)
                @if(!isset($transaction->traceNumber))
                    ...
                    <button class = "btn purple completeTransactionInfo transactionSpecialButton transactionSpecialButton_{{$transaction->id}}" data-role = "{{$transaction->id}}" data-action = "{{action("Web\TransactionController@completeTransaction" , $transaction->id)}}" data-target = "#completeTransactionInfo" data-toggle = "modal">
                        وارد کردن کد تراکنش
                    </button>
                @endif
            @endif

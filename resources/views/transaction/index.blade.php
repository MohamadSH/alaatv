...
@foreach($transactions as $transaction)
    <tr id="{{$transaction->id}}">
        <th></th>
        <td >@if(isset($transaction->order->user->id)) @if(strlen($transaction->order->user->getFullName("lastNameFirst")) > 0) <a target="_blank" href="{{action("UserController@edit" , $transaction->order->user)}}">{{$transaction->order->user->getFullName("lastNameFirst")}}</a> @else <span class="label label-sm label-danger"> درج نشده </span> @endif @endif</td>
        ...
        <td>@if(isset($transaction->order->user)) @if(!isset($transaction->order->user->mobile) ) <span class="label label-sm label-danger"> درج نشده </span> @else {{$transaction->order->user->mobile}} @endif @endif</td>
        @permission((Config::get('constants.SHOW_TRANSACTION_TOTAL_COST_ACCESS')))
        <td>@if(isset($transaction->order->cost) || isset($transaction->order->costwithoutcoupon)){{number_format($transaction->order->totalCost())}} @else <span class="label label-sm label-danger"> درج نشده </span>  @endif</td>
        ...
        <td>@if(isset($transaction->created_at) && strlen($transaction->created_at) > 0){{ $transaction->CreatedAt_Jalali() }}@else <span class="label label-sm label-danger"> درج نشده </span> @endif</td>
        <td>@if(isset($transaction->deadline_at) && strlen($transaction->deadline_at) > 0){{ $transaction->DeadlineAt_Jalali() }}@else <span class="label label-sm label-default"> ندارد </span> @endif</td>
        ...
        @endpermission
            @if($transaction->cost < 0)
                @if(!isset($transaction->traceNumber))
                    <a target="_blank" class="btn red transactionToDonateButton transactionSpecialButton_{{$transaction->id}}" data-role="{{$transaction->id}}"  data-action="{{action('TransactionController@convertToDonate' , $transaction)}}">تبدیل به دونیت</a>
                    <button class="btn purple completeTransactionInfo transactionSpecialButton transactionSpecialButton_{{$transaction->id}}"  data-role="{{$transaction->id}}"   data-action="{{action("TransactionController@completeTransaction" , $transaction->id)}}" data-target="#completeTransactionInfo" data-toggle="modal">
                         وارد کردن کد تراکنش </button>
                @endif
            @endif
        </td>
        <td >@if(isset($transaction->order->managerDescription) && strlen($transaction->order->managerDescription)>0 || isset($transaction->managerComment) && strlen($transaction->managerComment)>0) @if(isset($transaction->order->managerDescription) && strlen($transaction->order->managerDescription)) {{$transaction->order->managerDescription}} @endif @if(isset($transaction->managerComment) && strlen($transaction->managerComment)>0) {{$transaction->managerComment}} @endif @else <span class="label label-sm label-warning"> درج نشده </span> @endif</td>
        {{--<td>--}}
            {{--<div class="btn-group">--}}

                                ...
                                @foreach($questions as $key => $question)
                                    <div class="tab-pane @if($key == 0) active @endif" id="tab_1_1_{{$key+1}}">
                                        @if(strcmp($question->control->name, "multiSelect") == 0)
                                            ...
                                            <p class="font-blue"  style="text-align: justify">{{$question->description}}</p>
                                        ...
                                        @endif
                                            <div class="col-md-12">
                                                <div class="table-scrollable">
                                                    ...
                                                    <thead >
                                                        <tr >
                                                            <th class="text-center"> ثبت شده ها:  <span id="answerTableHeaderCounterNumber_{{$question->id}}">{{count($answersData[$question->id])}}</span><img class="hidden"  id="answerTableHeaderCounterLoading_{{$question->id}}" src="/assets/layouts/layout2/img/loading-spinner-blue.gif" style="height: 25px"></th>
                                                            {{--<th class="text-center">دلیل انتخاب</th>--}}
                                                        </tr>
                                                    </thead>
                                                    <tbody class="text-center">
                                                        @if(empty($answersData[$question->id]))
                                                            <tr class="text-center bold font-red"><td colspan="2">شما تاکنون موردی را درج ننموده اید</td></tr>
                                                        @else
...
//                            alert('Changed option ' + $(option).val() + '.');
                            ...
                            $.ajax({
                                ...
                                data: {event_id:{{$event->id}} , survey_id:{{$survey->id}} , question_id:question_id , answer : answer },
                                statusCode: {
...
//                                        console.log(response.responseText);
                            ...
                            $.ajax({
                                ...
                                data: {event_id:[{{$event->id}}] , survey_id:[{{$survey->id}}] , question_id:[question_id] },
                                statusCode: {
                                    //The status for when action was successful
                                    200: function (response) {
                                        var querySourceUrl ;

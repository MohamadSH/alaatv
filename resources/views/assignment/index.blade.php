                    ...
                    <div class = "timeline-body-head">
                        ...
                        <div class = "timeline-body-head-actions">
                            @if(strlen($assignment->questionFile)>0 || strlen($assignment->solutionFile)>0 || strlen($assignment->analysisVideoLink)>0)
                                <div class = "btn-group">
                                    <button class = "btn btn-circle green btn-sm dropdown-toggle" type = "button" data-toggle = "dropdown" data-hover = "dropdown" data-close-others = "true">
                                        دانلود
                                    ...
                                    <ul class = "dropdown-menu pull-right" role = "menu">
                                        @if(strlen($assignment->questionFile)>0)
                                            <li>
                                                <a href = "{{action("Web\HomeController@download" , ["content"=>"تمرین","fileName"=>$assignment->questionFile ])}}">دانلود تمرین</a>
                                            </li>
                                        @endif
                                        @if(strlen($assignment->solutionFile)>0)
                                            <li>
                                                <a href = "{{action("Web\HomeController@download" , ["content"=>"پاسخ تمرین","fileName"=>$assignment->solutionFile ])}}">دانلود کلید تمرین</a>
                                            </li>
                                        @endif
...
@else
    ...
    @foreach($assignments as $assignment)
        <tr>
            <th></th>
            <td>@if(isset($assignment->name) && strlen($assignment->name)>0) {{ $assignment->name }} @else
            ...
            <td>@if(isset($assignment->numberOfQuestions) && strlen($assignment->numberOfQuestions)>0) {{ $assignment->numberOfQuestions }} @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
            <td>@if(isset($assignment->questionFile) && strlen($assignment->questionFile)>0)
                    <a href = "{{action("Web\HomeController@download" , ["content"=>"تمرین","fileName"=>$assignment->questionFile ])}}">
                        دانلود
                    </a> @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
            <td>@if(isset($assignment->solutionFile) && strlen($assignment->solutionFile)>0)
                    <a href = "{{action("Web\HomeController@download" , ["content"=>"پاسخ تمرین","fileName"=>$assignment->solutionFile ])}}">
                        دانلود
                    </a> @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif</td>
            <td>@if(isset($assignment->analysisVideoLink) && strlen($assignment->analysisVideoLink)>0)
                    <a href = "{{ $assignment->analysisVideoLink }}"> دانلود</a> @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif </td>
            <td>
            ...
            <td class = "center">@if(isset($assignment->created_at) && strlen($assignment->created_at)>0) @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif {{ $assignment->CreatedAt_Jalali() }}
            </td>
            <td class = "center">@if(isset($assignment->updated_at) && strlen($assignment->updated_at)>0) @else
                    <span class = "m-badge m-badge--wide label-sm m-badge--danger"> درج نشده </span> @endif {{ $assignment->UpdatedAt_Jalali() }}
            </td>
            <td>
                <div class = "btn-group">
                    ...
                    <ul class = "dropdown-menu" role = "menu">
                        ...
                        @endpermission @permission((Config::get('constants.REMOVE_ASSIGNMENT_ACCESS')))
                        <li>
                    ...
                    <!-- static -->@permission((Config::get('constants.REMOVE_ASSIGNMENT_ACCESS')))

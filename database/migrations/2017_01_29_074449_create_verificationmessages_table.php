...
class CreateVerificationmessagesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('verificationmessages', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger("user_id")->comment("آی دی مشخص کننده کاربر گیرنده پیام");
            $table->string("code")->nullable()->comment("کد ارسال شده به کاربر");
            $table->unsignedInteger("verificationmessagestatus_id")->nullable()->comment("وضعیت پیام");
            $table->dateTime("expired_at")->nullable()->comment("زمان انقضای کد ارسال شده");
            $table->timestamps();

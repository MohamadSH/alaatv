...
class CreateUseruploadsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('useruploads', function (Blueprint $table) {
            ...
            $table->string("title")
                  ->nullable()
                  ->comment("عنوان وارد شده کاربر برای این آپلود");
            $table->longText("comment")
                  ->nullable()
                  ->comment("توضیح وارد شده کاربر درباره آپلود");
            $table->longText("staffComment")
                  ->nullable()
                  ->comment("توضیح مسئول درباره فایل");
            $table->tinyInteger("isEnable")
                  ->default(1)
                  ->comment("فعال / غیر فعال");
            $table->unsignedInteger("useruploadstatus_id")
                  ->nullable()
                  ->comment("وضعیت فایل");
            $table->timestamps();
            $table->softDeletes();

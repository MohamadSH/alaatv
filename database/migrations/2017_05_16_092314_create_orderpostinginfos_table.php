...
class CreateOrderpostinginfosTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('orderpostinginfos', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('order_id')->comment("آی دی مشخص کننده سفارش این پست");
            $table->unsignedInteger('user_id')->comment("آی دی مشخص کننده مسئول درج کننده اطلاعات پستی");
            $table->string('postCode')->nullable()->comment("کد پست (شماره مرسوله)");
            $table->timestamps();
            $table->softDeletes();

...
class AlterTableOrderproductsAddColumns extends Migration
{
    ...
    public function up()
    {
        Schema::table('orderproducts', function ($table) {
            $table->unsignedInteger('orderproducttype_id')->nullable()->comment("آیدی مشخص کننده نوع آیتم سبد")->after('id');
            $table->double('discountPercentage')->default(0)->comment("تخفیف این آیتم سبد(به درصد)")->after('cost');
            $table->integer('discountAmount')->default(0)->comment("تخفیف این آیتم سبد(مبلغ)")->after('discountPercentage');

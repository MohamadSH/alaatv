...
class Employeetimesheet extends Model
{
    ...
    {
        if (!isset($value)) return "00:00:00";
        else return $value;
    }
...
//        $explodedTime = $explodedDateTime[1] ;
    ...
    public function getDate($mode)
    {
        switch ($mode) {
            case "WEEK_DAY":
                return $this->convertToJalaliDay(Carbon::parse($this->getOriginal("date"))->format('l'));
            default:
                break;
        }
        ...
        switch ($mode) {
            ...
            case "PERSIAN_FORMAT":
                if ($this->obtainRealWorkTime("HOUR_FORMAT") !== false && $this->obtainShiftTime() !== false) {
                    ...
                    $second = $shiftTime[2];
                    $persianTime = "";
                    if ($hour > 0) $persianTime .= $hour . " ساعت ";
                    if ($minute > 0) $persianTime .= $minute . " دقیقه ";
                    if ($second > 0) $persianTime .= $second . " ثانیه ";
            ...
            case "PERSIAN_FORMAT":
                if (isset($this->userBeginTime) && isset($this->userFinishTime)) {
                    ...
                    if (strcmp($shiftTime, "00:00:00") == 0) {
                        ...
                        $second = $shiftTime[2];
                        $persianShiftTime = "";
                        if ($hour > 0) $persianShiftTime .= $hour . " ساعت ";
                        if ($minute > 0) $persianShiftTime .= $minute . " دقیقه ";
                        if ($second > 0) $persianShiftTime .= $second . " ثانیه ";

...
class CouponController extends Controller
{
        ...
        if ($request->has("validSinceEnable")) {
            if ($request->has("validSince") && strlen($request->get("validSince")) > 0) {
                ...
                if (strlen($sinceTime) > 0) {
                    $sinceTime = Carbon::parse($sinceTime)->format('H:i:s');
                } else {
                    $sinceTime = "00:00:00";
                }
        ...
        if ($request->has("validUntilEnable")) {
            if ($request->has("validUntil") && strlen($request->get("validUntil")) > 0) {
                ...
                if (strlen($untilTime) > 0) {
                    $untilTime = Carbon::parse($untilTime)->format('H:i:s');
                } else {
                    $untilTime = "00:00:00";
                }
    ...
    public function edit($coupon)
    {
        ...
        return view('coupon.edit',
            compact('coupon', 'limitStatus', 'defaultLimitStatus', 'coupontype', 'products', 'couponProducts', 'validSinceDate', 'validSinceTime',
